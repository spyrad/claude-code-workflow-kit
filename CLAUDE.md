...
- **Idea management**: `idea` (quick capture by the human, always writes `INBOX.md`), `idea-review` (triage of the main inbox), `idea-rank` (read-only effort×value ranking of the open `INBOX.md` ideas — sorts each into exactly one of four pots (Quick Wins / strategisch wertvoll / wartend/blockiert / braucht eigenen Fokus) by a fixed rule order, lists dependencies and recommends an order; chat report only, `produces: []`, optional precursor of `idea-review`. Not to be confused with `idea-triage`, which writes and works on the basin), `idea-triage` (sifts the machine basin `INBOX-BEFUNDE.md` portion by portion — all new entries plus all due ones plus at most `altbestand_pro_lauf` old ones, oldest first — and decides per entry: promote to `INBOX.md`, discard to `archive/INBOX-BEFUNDE-verworfen.md` with a mandatory reason, or leave lying exactly once, after which the next run forces the decision. Every decision is written immediately, never collected at the end)
