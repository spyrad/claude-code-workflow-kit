...
- **Session lifecycle**: `workflow-checkpoint` (since 2026-09-23 also compares the previous `## Offene Aufgaben` list before writing: every dropped task gets exactly one note under `### Aufgaben-Abgaenge` in the session log, every open task carries `(seit YYYY-MM-DD)`, tasks older than `status.alter_schwelle_tage` get ONE ⏳ decision template — DSR §9), `workflow-resume` and `workflow-next` (both show an `In Worktrees` block: one read-only line per linked Worktree with kind, branch, state, uncommitted count and progress; `frisch`/`gemergt → aufraeumen`/`verwaist` — DSR §10), `workflow-status`, `session-summary` (read-only recap of active session / a given day / last 7 days), `no-loss-check` (read-only detector running as step 0 of `workflow-checkpoint`: compares the conversation against the artifact state and reports lessons, subject-matter questions and ideas that were never captured — advisory, never blocking, writes nothing itself; the findings of its group "Vor dem Checkpoint erledigen" are written by `workflow-checkpoint` step 0 into `project-rules/lessons.md`/`INBOX.md` after a confirmed collection template, "Kann warten" stays commands in the report)
- **Idea management**: `idea` (quick capture by the human, always writes `INBOX.md`), `idea-review` (triage of the main inbox), `idea-rank` (read-only ranking of the open `INBOX.md` ideas as ONE table sorted by importance — short title, effort as a time range, importance in six levels / four colors, a remark cell carrying blockers and dependencies; the row order is the recommendation; chat report only, optional precursor of `idea-review`), `idea-triage` (sifts the machine basin `INBOX-BEFUNDE.md` portion by portion — all new entries plus all due ones plus at most `altbestand_pro_lauf` old ones, oldest first — and decides per entry: promote to `INBOX.md`, discard to `archive/INBOX-BEFUNDE-verworfen.md` with a mandatory reason, or leave lying exactly once, after which the next run forces the decision. Every decision is written immediately, never collected at the end)
...
- **Maintenance**: `archive` (move completed/discarded items to archive; an idea `Ausgearbeitet` without a valid change link is reported as "Change fehlt" instead of archived — DSR §8; lines under `## Offene Aufgaben` are left for the checkpoint), `kit-sync` (install/update installed kit copies under `~/.claude/` via lock-based drift detection — see below)
- **Development**: `impl-review` (feature-end review across plan drift, craft, rules — the former rules-only review, now retired), `backlog-status` (incl. an `In Worktrees` section after the active features, same Worktree rule as the session views), `repo-sync`, `build-check` (stand-alone deploy-readiness check across all repos — not part of the feature loop), `commit-and-push` (guided routine commit+push; groups paths by their real git root via `rev-parse --show-toplevel`, not `config.repos` — monorepo/poly-repo/single safe; hard safety rules, never `--force`/`--no-verify`/`--amend`)
- **Autonomous execution**: `worker` (two modes: view mode scans INBOX ideas and open tasks and judges per entry whether a background worker could complete it without the full chain — 4 criteria, pre-questions side column, dependency/order analysis; run mode, only after explicit approval, executes entries as background workers in isolated worktrees — checking off `## Schritte` in the task's own change folder and writing a status-neutral `worker-report.md` per task; a second carrier runs workers as visible Herdr pane sessions that commit on their own task branch — never push — and return a hand-off block instead of a report; after the start the orchestrator offers an optional read-only monitoring tick (`#### Rueckweg: Ueberwachungs-Tick` — finds all pane workplaces itself via `git worktree list` + `herdr agent list`, checks pane status, worktree stand, hand-off; reports only changes, ends itself once no workplace is left; `/loop 15m` suggested for Claude Code, fallback "Stand?" elsewhere — never started by the skill itself). Neither carrier touches central files or status displays; capped via `worker:` block in `workflow.config.yaml`, defaults 3 attempts / 30 minutes. Autonomy holds only between approval and acceptance — rule in `skills/CLAUDE.md`), `pane-start` (interactive counterpart to the worker's pane carrier: sets up a full-lane workplace for ONE change — own worktree on `feature/{slug}`, own Herdr pane, started and detected Claude session — and delivers ONE greeting message carrying workplace, suggested start command, orchestrator address and the return-path duty, ending with an explicit wait obligation: the session greets and waits for the human, it never runs the suggested command itself. Reuses the worker's Herdr start sequence by reference, never by copy (structure check with two separate failure paths guards the source); hard Herdr gate (`HERDR_ENV=1` + CLI + own pane address), full worktree guard despite empty `produces` (worktrees are created from the main checkout only). NO autonomy governance — no attempt cap, no time cap, no report: a human works in the pane. After delivery it offers the worker's Ueberwachungs-Tick by reference (structure check 3/3 incl. the tick anchor). Writes no artifacts and no central files; the return path is the unchanged WORKTREE-HANDOFF block via `dtb:workflow-checkpoint`)
- **Knowledge management**: `docs-extract` (scan input/ folder, extract facts from documents into thematic MDs), `lesson` (capture a reusable lesson append-only into `project-rules/lessons.md`, read as a prior by impl-plan/debug-plan/plan-review/impl-review — replaces the dead `pitfalls.md` concept), `open-question` (active capture: appends a subject-matter question `- [ ] [Fach] {Frage}` §6-conform to the active feature's `## Offene Punkte` — Variante-c target routing + Variante-B confirm; the capture tool for the Fach-Frage convention #13, read by `meeting-agenda`), `meeting-agenda` (read-only pre-meeting view: collects open `- [ ] [Fach]` questions from `features/*/{discovery,spec}.md` — only inside `## Offene Punkte`, outside code fences, `archive/` excluded; same two files the rest of the chain serves — and groups them by feature; `alle` argument adds answered questions and the full interim-status history. The reading end of the Fach-Frage chain #13→#26→#25→#24)
...
- `dtb-project/project-workflows/features/<slug>/` — **Change-Folder-Modell:** one folder per change (kebab-case slug), fixed filenames inside — `discovery.md`, `spec.md`, `plan.md`, optional `bug.md`/`task.md` (plus the status-neutral `review.md` from `dtb:impl-review`, the transient `fast-draft.md` from `dtb:feature-fast` and the status-neutral `worker-report.md` from `dtb:worker`). Replaces the former flat `FEATURE_*.md`/`PLAN_*.md`/`DISCOVERY_*.md`/`BUG_*.md`/`TASK_*.md` files paired by name. Status is derived from files present in the folder + `## Progress` in `plan.md` (see `DERIVED_STATE_RULES.md`)
...
- `dtb-project/project-workflows/INBOX-BEFUNDE.md` — **Befund-Becken:** machine-captured loss findings written by `dtb:workflow-checkpoint` step 0. The split is structural, not a field: `dtb:idea` is `disable-model-invocation` and therefore always typed by the human (→ `INBOX.md`), the checkpoint writes machine findings (→ the basin). Shared numbering across both files, the number survives promotion. A basin entry is **never worked on** until promoted — no worker picks it up, no work view shows it (status-neutral, `DERIVED_STATE_RULES.md` §6.4)
- `dtb-project/project-workflows/archive/INBOX-BEFUNDE-verworfen.md` — append-only log of discarded findings with their reason; the searchable ground when a topic comes back
