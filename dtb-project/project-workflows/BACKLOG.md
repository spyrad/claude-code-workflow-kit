...
| Pipeline-Kanten reziprok nachtragen | Offen | Mittel | features/pipeline-kanten-reziprok/task.md | Die zwei echten halbseitigen Kanten aus INBOX #46 gegenrichtig eintragen (`plan-review.after` += `feature-fast`, `feature-discover.after` += `greenfield-roadmap`); die vier read-only-Faelle und die Konventionsfrage bleiben bewusst draussen |
