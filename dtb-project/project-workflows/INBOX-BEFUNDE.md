...
|---|-------|--------|----------|
| 83 | 2026-09-09 | **Die INBOX braucht einen Status fuer nebenbei umgesetzte Ideen.** Heute gibt es vier Werte (`Offen` / `In Arbeit` / `Ausgearbeitet` / `Verworfen`); eine ohne eigenes Feature erledigte Idee passt in keinen: `Ausgearbeitet` verspricht eine `spec.md`, die es nicht gibt (und `dtb:project-health` prueft genau diesen Link), `Verworfen` heisst „bewusst nicht weiterverfolgt". Belegt 2026-09-09 an #74 und #73 — beide im Rahmen von `ideen-becken` umgesetzt, beide mussten mit Erklaervermerk auf `Verworfen`. Beim naechsten `/dtb:archive`-Lauf sind sie nicht mehr von echten Ablehnungen unterscheidbar. Zu klaeren: (a) fuenfter Wert `Erledigt` vs. Vermerk-Konvention im Textfeld; (b) wer ihn setzt (idea-review) und wer ihn liest (archive, project-health, worker); (c) Verhaeltnis zu #44 (`Ausgearbeitet` hat keine Leseansicht) |  |
